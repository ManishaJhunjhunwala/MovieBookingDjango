...
from .forms import SearchForm, BookingForm, SeatsForm2, SeatsForm, CreateUserForm, CancelForm
import datetime
...
def logoutUser(request):
    try:
        # first we logout the user then redirect it to the homepage
        logout(request)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            shows = theater_movies_rel.objects.filter(
                movie_id=movieid, theater_id__in=theaters, show_start__date=datestring).order_by('theater_id')
...
def BookingSuccess(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            umail = request.session.get('email')
            userob = User.objects.get(email=umail)
...
def myBookings(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            print("form is valid")
            bookingid = form.cleaned_data['bookedid']
            cancelseats = user_seats.objects.filter(user_bookings_id=bookingid)
            for seat in cancelseats:
    ...
    #  bookedseats = user_seats.objects.select_related('theater_seats_id').filter(
    #             theater_seats_id__theater_movies_id=theaterrelid, status=confirm).values_list('theater_seats_id__seat_no', flat=True)
    userseats = user_seats.objects.select_related(
        'user_bookings_id', 'theater_seats_id', 'theater_seats_id__theater_movies_id', 'theater_seats_id__theater_movies_id__movie_id', 'theater_seats_id__theater_movies_id__theater_id', 'theater_seats_id__theater_movies_id__theater_id__city_id').filter(user_bookings_id__in=bookinglist, status="confirmed").values_list('user_bookings_id', 'theater_seats_id__theater_movies_id__movie_id__movie_name', 'theater_seats_id__theater_movies_id__theater_id__theater_name', 'theater_seats_id__theater_movies_id__theater_id__city_id__city_name', 'status',                                    'theater_seats_id__theater_movies_id__movie_id__url').distinct()
    print(userseats)
    # print(userseats.query)
    booking_list = []
    for bookings in userseats:
        form_list = []
        form2 = CancelForm(initial={'bookedid': bookings[0]})
        seatnum = user_seats.objects.select_related(
            'theater_seats_id').filter(user_bookings_id=bookings[0]).values_list('theater_seats_id__seat_no', flat=True)
        seatnum = list(seatnum)

        form_list.append(form2)
        form_list.append(bookings)
        form_list.append(seatnum)
        booking_list.append(form_list)
    movielist = movie.objects.select_related()
    # print(movielist.query)

    context = {'name': userob.username, 'bookinglist': booking_list}
